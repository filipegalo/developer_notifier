    ...
    <div class="px-4 py-5 sm:px-6">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center">
          <img src="/images/github.svg" alt="GitHub Icon" class="w-6 h-6 mr-2">
          <h2 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">GitHub Pull Requests</h2>
        </div>
...
const refreshPullRequests = async () => {
  ...
  try {
    ...
    pullRequests.value = data.pull_requests;
...
onMounted(async () => {
  emit('loading', 'PullRequestsList', true);
  await refreshPullRequests();
  emit('loading', 'PullRequestsList', false);
});
